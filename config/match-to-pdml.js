...
function parsePDML(input) {
    ...
    for (let line of lines) {
        if (line.startsWith("!")) {
            ...
            let map = parts[0];
            let timeLeft = parts[1] ? ` timeLeft="${parts[1]}"` : "";
            let gameMode = parts[2] ? ` gameMode="${parts[2]}"` : "";
            let server = parts[3] ? ` server="${parts[3]}"` : "";
            
            output += `\t<match map="${map}"${timeLeft}${gameMode}${server}>\n`;
            currentMatch = map;
