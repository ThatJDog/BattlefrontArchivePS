...
async function processSeriesFile(seriesData, db) {
    ...
    const teamNames = seriesData.hasAttribute('teams') 
    ? seriesData.getAttribute('teams').map(team => team.value)
    : [];
    ...
    seriesData.children.forEach((matchNode, matchIndex) => {
        ...
        matchNode.children.forEach(teamNode => {
            let teamScore = teamNode.getOrDefaultAttribute('score', 0);
        ...
        matchNode.children.forEach((teamNode, teamIndex) => {
            ...
            var teamName = (teamNames && teamNames.length > teamIndex) 
            ? teamNames[teamIndex]
            : teamName = '';
            ...
            db.insert('TeamScore', {
                ...
                Won: teamNode.getOrDefaultAttribute('score', 0) == bestTeamScore
            });
